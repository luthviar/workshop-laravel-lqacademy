...
<section class="py-5">
    <div class="container px-4 px-lg-5 my-5">
        <div class="row gx-4 gx-lg-5 align-items-center">
            ...
            <form action="{{ route('product.update', $product->id) }}" method="post" class="col-lg-8">
                @csrf
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="url_image" name="url_image" value="{{$product->url_image}}" placeholder="URL Gambar Produk..">
                </div>
                <div class="mb-3">
                    <label for="price" class="form-label">Harga Produk</label>
                    <input type="number" class="form-control" id="price" value="{{$product->price}}" name="price" placeholder="Harga Produk.." required>
                </div>
                <div class="mb-3">
                    ...
                    <select class="select2 form-control" name="categories[]" multiple="multiple">
                        @foreach($categories as $category)
                            <option value="{{ ($category->id) }}" {{ (in_array($category->id,$idCategories)?'selected':'') }} >{{$category->name}}</option>
                        @endforeach
                    </select>              
